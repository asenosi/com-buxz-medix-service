...
export function AppointmentWizard({ open, onOpenChange, appointment, onAppointmentCreated }: AppointmentWizardProps) {
  ...
  const onSubmit = async (values: z.infer<typeof appointmentSchema>) => {
    ...
    try {
      ...
      let practitionerId = values.practitioner_id === "none" || !values.practitioner_id ? null : values.practitioner_id;

      // If manual entry with doctor details but no practitioner selected, create new practitioner
      if ((!practitionerId || practitionerId === "none") && values.doctor_name && values.doctor_name.trim()) {
      ...
      <DialogContent className="max-w-lg p-0 gap-0 overflow-hidden">
              ...
              <div className="flex-1 flex items-center justify-center overflow-auto">
                <Calendar
                  ...
                  classNames={{
                    ...
                    nav_button: cn(
                      "h-9 w-9 bg-background hover:bg-muted rounded-full p-0 shadow-sm border border-border/50 transition-all",
                    ),
                    nav_button_previous: "",
                    nav_button_next: "",
                    table: "w-full border-collapse space-y-1 mt-4",
                    head_row: "flex justify-between mb-2",
                    head_cell: "text-muted-foreground w-10 font-medium text-xs uppercase",
                    row: "flex justify-between w-full mt-1.5",
                    cell: "h-10 w-10 text-center text-sm p-0 relative flex items-center justify-center",
                    day: cn(
                      "h-10 w-10 p-0 font-normal rounded-full transition-all hover:bg-muted",
          ...
          <div className="flex flex-col h-[600px]">
            ...
            <div className="flex-1 overflow-y-auto px-6 pb-6 space-y-4">
              <div>
                ...
                />
                <p className="text-xs text-muted-foreground">{form.watch("title")?.length || 0}/50 characters</p>
              </div>
        ...
        {step === 5 && (
          <div className="flex flex-col h-[600px]">
            ...
            <div className="px-6 pb-6 mt-4 shrink-0">
              <Button
